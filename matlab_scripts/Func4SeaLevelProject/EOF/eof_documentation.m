...
% information was being taken out of our |sst| dataset.  So let's make a gif using my <https://www.mathworks.com/matlabcentral/fileexchange/63239-gif
% |gif|> function: 
% 
%  % First frame:
%  figure
...
%  gif('SeasonalTemperatureAnomalies.gif','frame',gcf,'DelayTime',0.2) 
%  
...
anomaly(t,-pc(1,:),'topcolor',[1 .35 .71],'bottomcolor',[.51 .58 1]) % First principal component is enso
box off
axis tight
datetick('x','keeplimits') 
...
%  caxis([-2 2]) 
%  cmocean balance
%  title(datestr(t(startind),'yyyy')) % initializes gif
%  
%  gif('SSTs_1990s.gif','frame',gcf)
%  
%  % Loop through the remaining fames: 
...
% insightful to the explained variance as a function of mode number, or as is also 
...
figure
subplot(1,2,1) 
imagesc(eof_maps_all(:,:,1)-eof_maps_10(:,:,1))
colorbar
cmocean('balance','pivot')
